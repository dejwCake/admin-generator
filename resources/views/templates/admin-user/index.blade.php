...
@foreach($columns as $col)
                                    <th is='sortable' :column="'{{ $col['name'] }}'"@if($col['name'] == 'activated') v-if="activation"@endif>{{'{{'}} __('admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</th>
@endforeach
...
@endforeach
	                    ...
	                    <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{'{{'}} $createUrl }}" role="button"><i class="fa fa-plus"></i>&nbsp; @{{ __('brackets/admin-ui::admin.btn.new') }} {{ $modelBaseName }}</a>
	                    </div>
                    </div>
                </div>
            </div>
