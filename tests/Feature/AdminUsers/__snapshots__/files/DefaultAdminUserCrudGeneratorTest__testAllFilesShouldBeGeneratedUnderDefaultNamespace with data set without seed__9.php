...
@section('title', __('admin.admin-user.actions.index'))
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ __('admin.admin-user.actions.index') }}
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="{{ $exportUrl }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{ __('admin.admin-user.actions.export') }}</a>
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ $createUrl }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ __('admin.admin-user.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    <th is='sortable' :column="'id'">{{ __('admin.admin-user.columns.id') }}</th>
                                    <th is='sortable' :column="'first_name'">{{ __('admin.admin-user.columns.first_name') }}</th>
                                    <th is='sortable' :column="'last_name'">{{ __('admin.admin-user.columns.last_name') }}</th>
                                    <th is='sortable' :column="'email'">{{ __('admin.admin-user.columns.email') }}</th>
                                    <th is='sortable' :column="'activated'" v-if="activation">{{ __('admin.admin-user.columns.activated') }}</th>
                                    <th is='sortable' :column="'forbidden'">{{ __('admin.admin-user.columns.forbidden') }}</th>
                                    <th is='sortable' :column="'language'">{{ __('admin.admin-user.columns.language') }}</th>
	                    ...
	                    <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ $createUrl }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ __('brackets/admin-ui::admin.btn.new') }} AdminUser</a>
	                    </div>
                    </div>
                </div>
            </div>
