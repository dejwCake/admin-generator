            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.categ-ory.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ $createUrl }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.categ-ory.actions.create') }}</a>
                    </div>
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.categ-ory.columns.id') }}</th>
                                        <th is='sortable' :column="'user_id'">{{ trans('admin.categ-ory.columns.user_id') }}</th>
                                        <th is='sortable' :column="'title'">{{ trans('admin.categ-ory.columns.title') }}</th>
                                        <th is='sortable' class="text-center" :column="'published_at'">{{ trans('admin.categ-ory.columns.published_at') }}</th>
                                        <th is='sortable' :column="'date_start'">{{ trans('admin.categ-ory.columns.date_start') }}</th>
                                        <th is='sortable' :column="'time_start'">{{ trans('admin.categ-ory.columns.time_start') }}</th>
                                        <th is='sortable' :column="'date_time_end'">{{ trans('admin.categ-ory.columns.date_time_end') }}</th>
                                        <th is='sortable' :column="'description'">{{ trans('admin.categ-ory.columns.description') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.categ-ory.columns.enabled') }}</th>
                                        <th is='sortable' :column="'send'">{{ trans('admin.categ-ory.columns.send') }}</th>
                                        <th is='sortable' :column="'price'">{{ trans('admin.categ-ory.columns.price') }}</th>
                                        <th is='sortable' :column="'views'">{{ trans('admin.categ-ory.columns.views') }}</th>
                                        <th is='sortable' :column="'created_by_admin_user_id'">{{ trans('admin.categ-ory.columns.created_by_admin_user_id') }}</th>
                                        <th is='sortable' :column="'updated_by_admin_user_id'">{{ trans('admin.categ-ory.columns.updated_by_admin_user_id') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="16">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/categ-ories')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('brackets/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('brackets/admin-ui::admin.listing.uncheck_all_items') }}</a>  </span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/categ-ories/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td class="text-center text-nowrap">
                                            <span v-if="item.published_at <= now">
                                                ...
                                                <span v-if="item.published_at > now">
                                                <small>{{ trans('admin.categ-ory.actions.will_be_published') }}</small><br />
                                                @{{ item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
