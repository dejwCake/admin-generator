            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="{{ $exportUrl }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{ trans('admin.category.actions.export') }}</a>
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ $createUrl }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.category.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'user_id'">{{ trans('admin.category.columns.user_id') }}</th>
                                        <th is='sortable' :column="'title'">{{ trans('admin.category.columns.title') }}</th>
                                        <th is='sortable' class="text-center" :column="'published_at'">{{ trans('admin.category.columns.published_at') }}</th>
                                        <th is='sortable' :column="'date_start'">{{ trans('admin.category.columns.date_start') }}</th>
                                        <th is='sortable' :column="'time_start'">{{ trans('admin.category.columns.time_start') }}</th>
                                        <th is='sortable' :column="'date_time_end'">{{ trans('admin.category.columns.date_time_end') }}</th>
                                        <th is='sortable' :column="'description'">{{ trans('admin.category.columns.description') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.category.columns.enabled') }}</th>
                                        <th is='sortable' :column="'send'">{{ trans('admin.category.columns.send') }}</th>
                                        <th is='sortable' :column="'price'">{{ trans('admin.category.columns.price') }}</th>
                                        <th is='sortable' :column="'views'">{{ trans('admin.category.columns.views') }}</th>
                                        <th is='sortable' :column="'created_by_admin_user_id'">{{ trans('admin.category.columns.created_by_admin_user_id') }}</th>
                                        <th is='sortable' :column="'updated_by_admin_user_id'">{{ trans('admin.category.columns.updated_by_admin_user_id') }}</th>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td class="text-center text-nowrap">
                                            <span v-if="item.published_at <= now">
                                                ...
                                                <span v-if="item.published_at > now">
                                                <small>{{ trans('admin.category.actions.will_be_published') }}</small><br />
                                                @{{ item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ $createUrl }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.category.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
