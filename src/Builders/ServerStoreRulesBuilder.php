...
final class ServerStoreRulesBuilder
{
    ...
    public function build(
        ...
        $this->serverStoreRules = new Collection();
        $this->buildServerStoreRulesByRequire($required);
        $this->buildServerStoreRulesByName($name);
        $this->getServerStoreRulesByUnique($name, $type, $unique, $tableName, $excludeDeletedAt);
        $this->getServerStoreRulesByType($majorType);

        return $this->serverStoreRules;
    }

    private function buildServerStoreRulesByRequire(bool $required): void
    {
        if ($required) {
    ...
    private function buildServerStoreRulesByName(string $name): void
    {
        if ($name === 'email') {
    ...
    private function getServerStoreRulesByUnique(
        string $name,
        string $type,
        bool $unique,
        string $tableName,
    ...
    private function getServerStoreRulesByType(string $majorType): void
    {
